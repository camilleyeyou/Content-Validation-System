...
export default function DashboardPage() {
  ...
  const fetchAll = React.useCallback(async () => {
    setError(null);
    try {
      const [approvedResp] = await Promise.all([
        apiGet<ApprovedRec[]>("/api/approved"),
      ]);
      setApproved(approvedResp);

      // /api/me can be 401 if not signed in yet
      try {
      ...
      // orgs may 401/403 if scopes missing — ignore silently
      try {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      {/* Settings block so users can plug their own LinkedIn app without redeploys */}
      <LinkedInAppSettings />
      ...
      <Card>
        <CardHeader
          ...
          actions={<ConnectLinkedInButton includeOrg variant="secondary">Re-connect</ConnectLinkedInButton>}
        />
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="text-sm text-zinc-600">
            Click <span className="font-medium">Generate</span> to run the pipeline. Approved posts
            will appear below. Select items and publish immediately or as drafts.
          </div>
        </CardContent>
      </Card>

      {/* Approved queue (same look/feel as Approved page, but condensed title) */}
      <Card>
        <CardHeader
                ...
                <label key={p.id} className="flex items-start gap-3 px-6 py-4 hover:bg-zinc-50 cursor-pointer">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-3">
                      <div className="font-medium break-words whitespace-pre-wrap">{p.content}</div>
